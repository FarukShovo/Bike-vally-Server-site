...
const uri = `mongodb+srv://${process.env.USER_NAME}:${process.env.USER_PASS}@cluster0.xgklx.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const database = client.db("bike-vally");
...
app.get("/", (req, res) => {
  res.send("starting my bike-vally");
});

app.listen(port, () => {
  console.log("Running my bike-vally on port", port);
});
